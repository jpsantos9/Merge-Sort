...
int main(){
    ...
    while (fileName!="exit"){
        ...
        //slit the process into 2 threads
        thread t1{&mergeSort, ref(list1), 0, list1.size()/2};   //thread for the first half
        thread t2{&mergeSort, ref(list1), list1.size()/2+1, list1.size()-1};    //thread for the second half
        t1.join();
        t2.join();
